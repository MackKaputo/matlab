function [Y, Z, extr_bl,extr_wstbl,nonpit]=pitsched(x,s,prod,dx,dy,dz)
...
X1=[];
for i=1:1:length(ys)
X=[ys(i,1)-j ys(i,2) ys(i,3)+l ys(i,1) ys(i,2) ys(i,3)+l ys(i,1)+j ys(i,2) ys(i,3)+l ys(i,1) ys(i,2)-k ys(i,3)+l ys(i,1) ys(i,2)+k ys(i,3)+l]; %%last colum is node mumber as on matrix p
X1=[X1;X];
end
clear X;
X2=[];
...
%%inficap1=[];
for jj=1:length(X2)
indi=find(z2(:,1)==X2(jj,1)& z2(:,2)==X2(jj,2)& z2(:,3)==X2(jj,3));
